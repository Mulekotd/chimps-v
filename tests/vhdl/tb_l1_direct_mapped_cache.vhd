...
architecture Behavioral of tb_l1_direct_mapped_cache is
    ...
    signal last_write_mask : STD_LOGIC_VECTOR(3 downto 0) := (others => '0');

    function backing_read(address : STD_LOGIC_VECTOR(31 downto 0)) return STD_LOGIC_VECTOR is
    begin
        case address(5 downto 2) is
            when "0000" => return x"11223344";
            when "0001" => return x"55667788";
            when "0010" => return x"99AABBCC";
            when "0011" => return x"DDEEFF00";
...
begin
    ...
    begin
        if rising_edge(clk) and memory_valid = '1' and memory_write = '1' then
            ...
            last_write_mask <= memory_write_mask;
        end if;
    end process;
    ...
    begin
        ...
        -- Escrita parcial em hit deve atualizar cache e backing (write-through).
        cpu_address <= x"00000004"; cpu_write <= '1'; cpu_write_data <= x"AAAABBBB";
        cpu_write_mask <= "0011"; cpu_valid <= '1';
        wait until rising_edge(clk) and cpu_ready = '1';
        cpu_valid <= '0'; cpu_write <= '0'; cpu_write_mask <= (others => '0');
        wait until rising_edge(clk) and cpu_ready = '1';
        assert last_write_data = x"AAAABBBB" and last_write_mask = "0011"
            report "L1 write-through byte mask failed" severity error;

        -- Uma linha conflitante no mesmo conjunto força novo miss/refill.
        cpu_address <= x"00000400"; cpu_valid <= '1';
        wait until rising_edge(clk) and cpu_ready = '1';
        cpu_valid <= '0';
        wait until rising_edge(clk) and cpu_ready = '1';
