...
Estou fazendo um trabalho da matéria de Arquiteturas de Computadores da faculdade, nele precisamos desenvolver uma microarquitetura do zero. Escolhemos RISC-V e o nome definido para o nosso projeto é "CHIMPS-V". Também, utilizaremos Docker para empacotar este projeto.
...
Tendo isso em mente, preciso que faça um research e estruture num arquivo CONTEXT.md todo processo de desenvolvimento que devemos seguir para este projeto. Lembre-se sempre de anotar todas as referências.

CONSTRAINTS:

- O projeto será desenvolvido seguindo padrões de Test Driven Development e Spec Driven Development;
- Prezamos por desempenho, então sempre que algo puder ser implementada como circuito digital, me informe;
- A microarquitetura deve ser capaz de trabalhar com pontos flutuantes utilizando o padrão IEEE 754;
- A CPU deve seguir o modelo de Von Neumann (Fetch, Decode, etc.);

FEATURES:

- CPU (ALU, FPU, Registradores, etc.)
...
A princípio é isso, sinta-se livre para agregar valor ao projeto com mais detalhes.
