...
Verifique se a minha implementação em VHDL dos circuitos atende as demandas que foram levantadas no escopo deste projeto. Depois disso, documente em um arquivo em docs o que cada circuito faz e como eles se comunicam entre si no final. Por fim, também documente como devemos seguir para os próximos passos de implementação: definir uma ISA, integrar diretamente com os circutos, etc.
