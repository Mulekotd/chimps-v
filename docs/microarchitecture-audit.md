...
L1 direta reutilizável, wrappers de I-cache/D-cache, fundação de ponto flutuante,
unidade M isolada e scripts Bash para GHDL em Docker. Ele contém PC, ALU, gerador de
imediatos, decoder, bancos de registradores inteiro/FP, RAM, núcleo single-cycle e
cache.
...
MMIO/FIFOs, integração da extensão M, ponto flutuante F/IEEE 754 completo, CSRs,
snapshots de ciclo, GUI desktop, carregador de imagens de programa, Docker e regressão
arquitetural.
...
|---|---|---|
| RTL em VHDL | Parcialmente atende | Há lint/compilação GHDL VHDL-2008 em scripts Bash e imagem Docker; ainda falta CI. |
| Perfil ISA | Parcialmente atende | `specs/isa-rv32i-chimps-v1.md` define RV32IMF_Zicsr, stack em software, CSRs/traps e instruções-alvo; o decoder ainda cobre apenas parte de RV32I. |
...
## 4. Problemas que devem ser resolvidos antes do pipeline
...
1. **Fechar o perfil ISA inicial.** Declarar um subconjunto verificável, por
   ...
   O perfil e a convenção de stack já estão documentados em
   `specs/isa-rv32i-chimps-v1.md`; falta implementá-los no decoder e no core.
