...
(Hennessy & Patterson, 5ª ed., cap. 5; cópia local em `docs/books`) e na semântica de
loads/stores byte-endereçados da especificação RV32I 2.1. Decisão: dados precedem
fetch, mas a política não é visível à ISA. Aceite: `tb_memory_bus` encaminha leitura,
escrita, máscara e resposta ao mestre correto. Contraprova: com ambas as portas
válidas, a resposta de instrução fica inativa e um `error` chega somente à porta de
dados selecionada.
