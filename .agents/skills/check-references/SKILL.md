...
- For VHDL implementations, consult the [IEEE Standard VHDL Language Reference Manual](https://edg.uchicago.edu/~tang/VHDLref.pdf) for language syntax and semantics, especially when the RTL depends on standard VHDL behavior. A copy may also be available locally under `/docs/books`; use the local copy when present and record the consulted edition or sections.
...
Some of the listed books may be available directly in the project under
`/docs/books`. Check that directory first and consult the relevant local copy
when available; otherwise use a user-provided copy or an accessible legitimate
source. Do not use credentials to access BRVirtual, assume its contents, or
reproduce substantial copyrighted text. If a cited source is unavailable,
state that the decision is based on the accessible specification and project
evidence rather than claiming verification against that book.
